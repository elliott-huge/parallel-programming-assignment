...
kernel void reduce_add_4(global const int* A, global int* B, local int* scratch) {
	...
	// TO IMPROVE THIS:
	//get workgroup number with get_group_id()
	//use workgroup number to index the output array (of length equal to no' of wgs)
	//use an additional kernel that uses this one's atomic method to complete the
	// operation
...
//mykernel
//none of these kernels use atomic add :)))
